...
// Handle klik baris agar checkbox tercentang otomatis
const handleRowClick = (event: any, { item }: any) => {
  const raw = item?.raw || item;
  if (selected.value.length > 0 && selected.value[0]?.Kode === raw.Kode) {
    selected.value = [];
  } else {
    selected.value = [raw];
  }
...
<template>
  <PageLayout title="Browse Data User" :menu-id="MENU_ID" icon="mdi-account-group-outline">
    <template #header-actions>
      <v-btn v-if="canInsert" size="small" color="primary" prepend-icon="mdi-plus"
        @click="router.push('/tools/users/baru')">Baru</v-btn>
      ...
      <v-btn size="small" variant="outlined" prepend-icon="mdi-file-excel" @click="handleExport">Export</v-btn>
      <v-btn size="small" variant="text" prepend-icon="mdi-close" @click="router.back()">Tutup</v-btn>
    </template>
    ...
    <div v-if="canView" class="browse-content">
      <div class="filter-section d-flex align-center bg-grey-lighten-4 pa-2 rounded mb-3 border">
        <div class="d-flex align-center mr-4">
          <div style="width: 16px; height: 16px; background-color: #d32f2f; margin-right: 4px;"></div>
        ...
        <v-text-field v-model="search" label="Cari..." density="compact" variant="outlined" hide-details
          prepend-inner-icon="mdi-magnify" style="max-width: 250px;" class="mr-2" />
        <v-btn @click="fetchData" color="primary" icon="mdi-refresh" variant="text" :loading="isLoading"
          size="small"></v-btn>
      </div>

      <div class="table-container">
        <v-data-table v-model="selected" :headers="headers" :items="masterData" :search="search" :loading="isLoading"
          item-value="Kode" select-strategy="single" return-object show-select density="compact" fixed-header
          class="desktop-table fill-height-table" :row-props="myRowProps" @click:row="handleRowClick"
          :items-per-page="50">

          <template #[`item.Aktif`]="{ item }">
            <span class="font-weight-bold">{{ item.raw?.Aktif }}</span>
          </template>
        </v-data-table>
      </div>
    </div>
    ...
    <v-dialog v-model="showConfirmDelete" max-width="400px">
      <v-card class="rounded-lg">
        ...
        <v-card-actions class="pa-4">
          <v-spacer></v-spacer>
          <v-btn variant="text" @click="showConfirmDelete = false" :disabled="isDeleting">Batal</v-btn>
          <v-btn color="error" variant="elevated" @click="executeDelete" :loading="isDeleting">Ya, Hapus</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </PageLayout>
