...
watch(
  ...
  (isOpen) => {
    if (isOpen) {
      if (props.isNewMode) {
        formData.value = { Kode: "", Nama: "", Alamat: "", Kota: "" };
...
const executeSave = async () => {
  ...
  try {
    ...
    await api.post("/perusahaan/save", payload);
      ...
      <v-form ref="formRef" @submit.prevent="handleSave">
        <v-card-title class="bg-primary text-white pa-4 d-flex align-center">
          ...
          <span class="text-subtitle-1 font-weight-bold">{{
            isNewMode ? "TAMBAH PERUSAHAAN" : "UBAH PERUSAHAAN"
          }}</span>
          <v-spacer></v-spacer>
          <v-btn
            icon="mdi-close"
  ...
  <v-dialog v-model="confirmDialogVisible" max-width="400px">
    <v-card class="rounded-lg">
      ...
      <v-card-text class="pa-4 pt-0"
        >Yakin ingin menyimpan data perusahaan ini?</v-card-text
      >
